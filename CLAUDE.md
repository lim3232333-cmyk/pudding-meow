...
Full-screen `#ivScreen` reached from the member grid (it took 会员明细's slot per Figma 207:104, so 会员明细 now hangs off the card's 成长值 label). Rewards are settled **only when the invited friend completes their first order** — registering alone pays nothing, otherwise a pile of throwaway signups farms coins. One settlement pays three parties: the inviter, the friend themselves, and the inviter's inviter (二级, derived from `members.referred_by`, no extra table). `referrals.first_order_rewarded` is grabbed as the idempotency lock (`UPDATE … RETURNING … INTO` + `FOUND`), so a retried HitPay webhook can't double-pay.
