...
returns table(
  ...
  usable_days jsonb, usable_hours jsonb, mall_image_url text)
language plpgsql
security definer
...
begin
  ...
  select mc.id, mc.coupon_id, c.name, c.type, c.value, c.min_spend,
         ...
         c.usable_days, c.usable_hours, c.mall_image_url
...
select count(*)                                                            as 已发出的券,
       ...
       count(*) filter (where c.mall_image_url is not null)                as 有券面图的
  from public.member_coupons mc
  join public.coupons c on c.id = mc.coupon_id;
